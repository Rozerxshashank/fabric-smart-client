...
func TestClient_StreamCallView(t *testing.T) {
	...
	// streamCommand error
	vsc.streamErr = fmt.Errorf("stream error")
	_, err = cl.StreamCallView("fid", []byte("input"))
...
func TestClient_ProcessCommand(t *testing.T) {
	...
	// Success
	resp, err := cl.processCommand(context.Background(), sc)
	require.NoError(t, err)
	require.NotNil(t, resp)

	// CreateViewClient error
	fc.createErr = fmt.Errorf("create view client error")
	_, err = cl.processCommand(context.Background(), sc)
	require.ErrorContains(t, err, "create view client error")
	fc.createErr = nil

	// unmarshal error
	vsc.respBytes = []byte("invalid-bytes")
	_, err = cl.processCommand(context.Background(), sc)
	require.ErrorContains(t, err, "failed to unmarshal")
	vsc.respBytes = nil

	// View error payload returned
	vsc.cmdRespErr = &protos.Error{Message: "view execution exception"}
	_, err = cl.processCommand(context.Background(), sc)
...
func TestClient_StreamCommand(t *testing.T) {
	...
	// CreateViewClient error
	fc.createErr = fmt.Errorf("create view client error")
	_, _, err := cl.streamCommand(context.Background(), sc)
	require.Error(t, err)
	fc.createErr = nil

	// Send error
	vsc.sendErr = fmt.Errorf("send error")
	_, _, err = cl.streamCommand(context.Background(), sc)
...
func TestClient_CreateSignedCommand(t *testing.T) {
	...
	// Randomness reader error
	cl.RandomnessReader = &failingReader{}
	payload := &protos.Command_CallView{CallView: &protos.CallView{Fid: "fid"}}
	_, err = cl.CreateSignedCommand(payload, sid)
	require.Error(t, err)
	cl.RandomnessReader = rand.Reader

	// Serialize error
	sid.serializeErr = fmt.Errorf("serialize error")
	_, err = cl.CreateSignedCommand(payload, sid)
	require.Error(t, err)
	sid.serializeErr = nil

	// Success path covering InitiateView payload mapping
...
func TestStream(t *testing.T) {
	...
	dummyConn, _ := grpc.NewClient("passthrough:///dummy", grpc.WithTransportCredentials(insecure.NewCredentials()))
	st := &Stream{
	...
	// Result recv error
	vsc.recvErr = fmt.Errorf("recv error")
	_, err = st.Result()
	require.ErrorContains(t, err, "recv error")
	vsc.recvErr = nil

	// Result unmarshal error
	vsc.respBytes = []byte("junk")
	_, err = st.Result()
	require.Error(t, err)
	vsc.respBytes = nil

	// Result view error
	vsc.cmdRespErr = &protos.Error{Message: "view error"}
	_, err = st.Result()
	...
	vsc.cvrBytes = nil
	vsc.respBytes = nil
	// Make it return empty init view resp instead
	...
	vsc.respBytes, _ = proto.Marshal(emptyResp)
	_, err = st.Result()
